...
public class OreGen {
	...
	public static void generateOres(final BiomeLoadingEvent event) {
		...
		if (!(event.getCategory().equals(Biome.Category.THEEND) || event.getCategory().equals(Biome.Category.NETHER))){
			addFeatureOverWorld(generation, BlockRegistry.coalLeanOre.get().getDefaultState(),
					17, 0, 128, 16);
			addFeatureOverWorld(generation, BlockRegistry.diamondLeanOre.get().getDefaultState(),
					8, 0, 16, 2);
			if (event.getCategory().equals(Biome.Category.TAIGA)) //针叶林
				addFeatureOverWorld(generation, BlockRegistry.emeraldLeanOre.get().getDefaultState(),
						6, 48, 80, 4);
			addFeatureOverWorld(generation, BlockRegistry.goldLeanOre.get().getDefaultState(),
					10, 0, 32, 6);
			addFeatureOverWorld(generation, BlockRegistry.ironLeanOre.get().getDefaultState(),
					15, 0, 128, 16);
			addFeatureOverWorld(generation, BlockRegistry.lapisLeanOre.get().getDefaultState(),
					10, 0, 16, 8);
			if (event.getCategory().equals(Biome.Category.MESA) || event.getCategory().equals(Biome.Category.DESERT)) //平顶山，沙漠
				addFeatureOverWorld(generation, BlockRegistry.quartzLeanOre.get().getDefaultState(),
						6, 16, 32, 6);
			addFeatureOverWorld(generation, BlockRegistry.redstoneLeanOre.get().getDefaultState(),
					10, 0, 16, 8);
		}
		//下届
		if (event.getCategory().equals(Biome.Category.NETHER)){
			addFeatureNether(generation, BlockRegistry.netheriteLeanOre.get().getDefaultState(),
					3, 0, 32, 10);
		}
		//末地
